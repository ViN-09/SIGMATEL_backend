...
class RequestTableStructureController extends Controller
{
    ...
    public function requestTableStructure($form)
    {
        ...
        if ($form == 'genset1') {
            ...
            $genset_latest = $db->table('genset1')->orderBy('id', 'desc')->first();
            $genset_columns = $db->select("SHOW COLUMNS FROM genset1");

            $report_genset = [];

            foreach ($genset_columns as $column) {
                ...
                if ($genset_latest && isset($genset_latest->{$column->Field})) {
                    $latestValue = $genset_latest->{$column->Field} ?? "-";
                }
            ...
            return response()->json([
                "report_genset" => $report_genset
            ]);
        }

        if ($form == 'genset2') {

            $genset_latest = $db->table('genset2')->orderBy('id', 'desc')->first();
            $genset_columns = $db->select("SHOW COLUMNS FROM genset2");
